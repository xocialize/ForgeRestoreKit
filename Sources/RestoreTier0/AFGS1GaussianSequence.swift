// AFGS1GaussianSequence.swift — RestoreTier0
//
// ✅ **The NORMATIVE AV1 `gaussian_sequence` table** — the 2048-entry constant the AV1 spec (§7.18.3)
// and AFGS1 define for film-grain synthesis. This closes the conformance gap `AFGS1Spec.swift`'s
// banner records: grain generated with this table is spec-conformant where the substitute was only
// shape-faithful.
//
// **Provenance (do not re-derive, do not retype):** transcribed mechanically from
// `dav1d/src/tables.c` (`dav1d_gaussian_sequence`), Copyright © 2018-2021 VideoLAN and dav1d
// authors, © 2018 Two Orioles, LLC — **BSD-2-Clause**, compatible with this package's MIT. The
// transcription was VERIFIED, not trusted: 2048 entries, mean 0.5, σ 511.5 (spec: N(0, 512)),
// range [-1752, 1688] ⊂ [-2048, 2047], every value a multiple of 4 — and the first/last entries are
// pinned in `AFGS1GaussianSequenceTests` so a silent corruption fails loudly. 🚨 The banner's
// warning stands: a hand-typed version of this table would look correct and be silently wrong —
// any future update must come from a machine transcription of a normative source.
//
// The substitute (`AFGS1.substituteGaussianSequence`) remains available for reproducing pre-0.11.0
// grain fields exactly.

extension AFGS1 {
    /// The normative AV1 `gaussian_sequence` (see file header for provenance and verification).
    public static let gaussianSequence: [Int16] = [
